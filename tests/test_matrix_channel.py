...
import leafbot.channels.matrix as matrix_module
...
from leafbot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("leafbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "leafbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("leafbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "leafbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
