...
2、【问题排查记录】Tavily 搜索不生效
   - 现象：配置了 tavilyApiKey，但 nanobot agent/gateway 运行时 web_search 返回
     "Search API not configured"，日志中无任何 Tavily 相关输出
   - 排查过程：
     a) 检查配置文件 ~/.nanobot/config.json → tavilyApiKey 已正确填写 ✓
     ...
     d) 在 web.py 的 execute() 方法开头加 debug 日志 → 运行 nanobot agent --logs 后
        该日志完全没有出现，说明执行的不是这个文件的代码
     e) 关键发现：pip show nanobot-ai 显示安装路径为 /root/project/nanobot
        而修改的代码在 /root/project/nanobot_my，两个是不同目录！
   - 根本原因：
     nanobot CLI 通过 pip install -e 安装自 /root/project/nanobot/（旧版），
     旧版 web.py 只有 Brave Search，完全没有 Tavily 回退逻辑。
     用户在 /root/project/nanobot_my/ 中做的改动（加入 Tavily 支持）从未被安装。
   - 修复：在 nanobot_my 目录下执行 pip install -e . 重新安装
